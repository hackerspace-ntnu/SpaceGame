"""Export the net gun's loaded variation to Unity.

Only `Coll_NetGun_Loaded` ships. It carries the net bundle as its own object,
`Mesh_NetGun_Bundle`, so the Unity prefab gets both states out of one FBX by
switching that object off — which is why `Coll_NetGun_Spent` has nothing to
export. The spent collection stays in the component file as the reference the
loaded gun is built against; adding it here is a line in TARGETS if something
ever needs a standalone spent mesh.

The two Marker_* cubes are exported WITH the mesh on purpose: they carry the
muzzle and grip coordinates across the FBX for the prefab to adopt and strip
(empties do not survive `object_types={"MESH"}`).
...
ever reads the .blend.
    ...
    blender --background --python components/props/net_gun_export.py
"""
...
TARGETS = [
    ("net_gun.blend", "Coll_NetGun_Loaded", ("Items", "net_gun.fbx")),
]
...
def export_one(blend_name, coll_name, fbx_parts):
    ...
    # unselected mesh into the file, and both variations sit on the same
    # origin — an unfiltered export ships two guns inside each other.
