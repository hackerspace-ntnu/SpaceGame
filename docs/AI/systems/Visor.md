...
symptoms:
  ...
  - "the target bracket and its name sit beside what I am pointing at, not on it"
  - "the bracket frames a whole ship or hull instead of the control under the crosshair"
  - "the bracket marks empty air in front of a machine I am aiming at"
  - "a readout writes a ValueText and the number never appears on screen"
...
| `VisorReticle` | [HelmetHUD/VisorReticle.cs](Assets/Game/Scripts/Presentation/UI/HelmetHUD/VisorReticle.cs) | Corner marks snapped around **the collider the `Interactor` arbitrated**, plus a look-at info box beside them from `InteractionPromptResolver` (label, value, prompt). `FramedSubject` and `MarksHitPoint` are static and testable. Absorbed `InteractionPromptUI`. The crosshair third of the design spec's `VisorReticle` is not built yet. |
...
**Looking at something:** `Interactor` resolves `HoveredInteractable` each frame **and publishes the collider and point it arbitrated** → `VisorReticle.LateUpdate` picks the subject with `FramedSubject` (walk up from the hit collider, stop at the interactable, take the first thing that draws anything), projects its renderer bounds, snaps the bracket onto it, and (separately) asks `InteractionPromptResolver.TryResolve` for the info box's label/value/prompt, fading it in and out on its own timer — see [InteractionSystem.md](InteractionSystem.md) for the resolver itself.

**Looking at something that is not an interactable:** nothing hovered → the reticle asks each `ICrosshairReadout` on the player's own body, in component order, and draws the first that answers. `WallAimController` is the only implementer.
...
- **The bracket marks the HIT, not the component — and that is not a detail.** It used to take the union of every `Renderer` under the `IInteractable` component, which is a different question from the one `Interactor` had already answered and went wrong three ways at once: an interactable reached through `GetComponentInParent` (how any collider with no component of its own answers) framed the whole hierarchy at its centroid; one whose collider is a **bare trigger standing proud of a fixture** — the only arrangement that lets a receptacle inside a solid body be aimed at, see [InteractionSystem.md](InteractionSystem.md) — had no renderers under it at all and fell back to the trigger's own origin, so the bracket marked exactly the padding that stands the trigger clear of the machine; and a component with children spread wide centred between them. `Interactor.HoveredCollider` / `HoveredPoint` are what fixed it. **Anything else that draws on a hovered target must read those two, not re-derive a position from the component.**
- **Too big to frame ⇒ mark the point.** `MarksHitPoint` compares the projected size against `maxSize`. Past it the corners clamp while the centre stays on the object's centroid, so a modest bracket floats in the middle of a hull the player is standing inside — the bracket stops being able to say "this thing, this big", and a mark on the aim point is the weaker claim and the true one.
- **The reticle must cancel `VisorSway`, and does.** The sway writes an `anchoredPosition` onto the visor ROOT every frame, so every descendant lags a few pixels behind a head turn. That is the point of the layer — except for a mark that claims to be *on* something in the world, which then drifts off its target exactly as the player swings the camera onto it. `TryToCanvas` goes through `RectTransformUtility.ScreenPointToLocalPointInRectangle` rather than dividing the screen point by the canvas scale, which takes the sway (and any other ancestor transform) out of the answer. Dividing by `UIScale.ScaleFactor` is correct only for a layer sitting at the canvas origin.
- **The reticle binds to the player it hangs under, not to `FindFirstObjectByType<Interactor>()`.** The old lookup returns an arbitrary body in a session with more than one player, so the bracket described what a stranger was looking at on a machine that could not act on it. Same rule and same resolver as the health gauge — `GameplayMenuScope.FindLocalPlayer(this)`.
- **`IInteractionReadout.ValueText` was written by three readouts and drawn by nothing** until the info box grew a right-aligned row for it beside the label. If you add a field to `InteractionDisplay`, grep for the thing that draws it before assuming it appears.
