...
symptoms:
  ...
  - "regenerating the robot settlement rewrites hundreds of scene lines with no code change"
  - "NPCs cannot walk on a rock or mesa I just generated"
  - "surface detail I dialled up does not show in the meshed feature"
reads_with: [WorldStreaming, NavMeshSystem, Environment, SceneTransitions, Towns]
updated: 2026-09-15
---
...
- **Settlements** = seeded height-map of grid cells → tile placements → prefab instantiation. Edit-time only, output lives in the scene. **Despite the name this builds ONE monumental ruin, not a town** — for towns see [Towns.md](Towns.md), whose `Town*` types are a separate family in `SpaceGame.World.Towns`.
...
- **Its footprint, clearance, ground-sampling and foundation-pad code now lives in [`TownPlacement`](Assets/Game/Scripts/World/ProceduralGeneration/Town/Core/TownPlacement.cs)**, shared with `TownGenerator`. The extraction is RNG-neutral — nothing in it draws a random number — so the Clanker town still generates identically. If you change it, re-verify by comparing the transform value multiset and `m_SourcePrefab` composition, **not** the raw scene diff (see the next entry).
- **Regenerating any settlement orphans every saved record in it, permanently, and the scene diff is therefore never clean.** `Clear()` + `Generate()` destroys and re-creates every instance, and `SaveableEntity.OnValidate` mints a fresh `instanceId` and `GlobalObjectIdHash` for each — so ~830 lines of identity churn even when nothing about the code changed (measured 2026-09-15 on pristine code). Those records are `authored`, so `DropVanishedRuntime` never removes them and they sit in every existing save file forever.
- **`RobotSettlementGenerator.Reroll` restores `useSeed` to its previous value *after* generating**, so a town you rerolled and liked cannot be reproduced by pressing Generate again. `TownGenerator.Reroll` deliberately keeps the new seed.
- **"the chunk scene should diff cleanly" is not a usable test for it.** `Clear` + `Generate` destroys and re-creates every instance, and `SaveableEntity.OnValidate` mints a fresh `instanceId` and `GlobalObjectIdHash` for each — so *any* regeneration rewrites ~830 lines of pure identity even with untouched code (829 measured on pristine HEAD, 2026-09-15). To prove a refactor did not alter generation, compare the transform value multiset and the `m_SourcePrefab` GUID composition instead. `TownGenerator` avoids the whole problem by reusing instances ([Towns.md](Towns.md)).
