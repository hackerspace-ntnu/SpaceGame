...
| A rope tied to terrain tears off in 0.2 s | `Leash.ResistSeconds` scales the base figure by the captor's **pull**, and `Leash.PullOf` returns 0 for anything with no engine — a wall, a rock, the lander, a crate. Every such end therefore hits the `0.1` ratio floor, giving `2 × 0.1 = 0.2 s` of pulling away to snap the rope. That inverts the intent stated in the doc ("tearing free of the lander is proportionally harder than tearing free of another player"): the most immovable things are the *easiest* to tear free of. Towing is no longer affected — `HeldBackFraction` gates strain on being genuinely held — but anchoring to scenery still is. The fix is to key the scale on the far end's **resistance** (mass, infinite for a static anchor) rather than its pull, with a cap so a wall stays escapable; the three existing `ResistSeconds` assertions in `LeashConstraintTests` pin the pull semantics and would need revising with it. | [LeashSystem](systems/LeashSystem.md) |
...
| `Camera.main` is never the player's view | The player camera on `PlayerCharacter.prefab` is `Untagged`, and the mount's orbit camera is explicitly set `Untagged` when spawned — so `Camera.main` resolves to whatever scene camera carries the tag, in every session, not just while mounted. Aim no longer reads it (`AimProvider` is the one answer, 2026-09-04), but the remaining users are presentation and are still wrong: `SnareCatch.Redraw` derives the net ribbon's winding from it and falls back to `Vector3.back`, which under ordinary culling renders the whole net as nothing; `GravelBlastFx`, `RepulsorGauntletArtifact` and `SuckerPuncherArtifact` measure shake proximity from it and skip the shake entirely when it is null. The fix is one shared "camera this machine is drawing from", not four more fallbacks. | [Artifacts](systems/Artifacts.md), [PlayerCharacter](systems/PlayerCharacter.md) |
