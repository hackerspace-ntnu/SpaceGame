...
| Tools/SpaceGame/Settlements/Build Mock Clanker Settlement (+ Bake NavMesh) | [ClankerSettlementBuilder.cs](Assets/Game/Editor/Environment/ClankerSettlementBuilder.cs) | Writes `ClankerSettlement.asset` (a `RobotSettlementRecipe`), finds the flattest 150 m disc 450–800 m from the spawn point that clears every terrain feature, places a seeded `RobotSettlementGenerator` under a `ClankerSettlement` root in that chunk scene, generates, saves. The `+ Bake NavMesh` variant then re-bakes the world NavMesh, which the new buildings made stale. Two `ClankerOutrider` riders go on the recipe's own outrider slots (`RobotSettlementRecipe.outriderPrefabs/outriderTotal`, placed last so the seed's layout is unchanged) and `Verify` refuses a town with fewer. The root also gets a `SettlementAlarm` and a `SettlementPopulation` (cap 26, three every 45 s as a band, Clankers 3:1 outriders); the garrison is 4–5 bands of 3–4 plus three outriders |
