...
def core(mats, coll):
    ...
    # Slim section. The body is a spine to hang wings off, not a hull — it
    # should read as light enough for something that flies by flapping.
    stations = [(-1.05, 0.062, 0.074), (-0.72, 0.115, 0.128),
                (-0.30, 0.150, 0.168), (0.10, 0.155, 0.173),
                (0.50, 0.134, 0.150), (0.85, 0.098, 0.112),
                (1.10, 0.066, 0.076)]
    ...
    for i in range(5):
        t = i / 4
        y = -0.80 + 1.75 * t
        hz = 0.173 - 0.08 * abs(t - 0.35) * 1.6
        p.box((0, y, hz + 0.020), (0.052, 0.34, 0.036), 1)

    # Flank chine seams and rivet rows.
    for sx in (-1, 1):
        p.seam((sx * 0.140, -0.62, 0.010), (sx * 0.140, 0.72, 0.010),
               width=0.022, depth=0.016, axis='X', mat=1)
        p.rivets((sx * 0.134, -0.55, 0.068), (sx * 0.134, 0.66, 0.068), 5,
                 radius=0.013, height=0.009, axis='X', mat=4)

    # Shoulder mounts: slim ribs rather than the slab bosses this had before.
    for sx in (-1, 1):
        p.box((sx * 0.152, -0.02, 0.020), (0.028, 0.26, 0.225), 5)
        p.rivets((sx * 0.168, -0.09, 0.110), (sx * 0.168, 0.07, 0.110), 2,
                 radius=0.015, height=0.010, axis='X', mat=4)

    # Belly rail — what the rider cradle hangs from.
    p.box((0, 0.02, -0.166), (0.078, 1.42, 0.042), 1)
    for y in (-0.56, -0.10, 0.36, 0.72):
        p.box((0, y, -0.188), (0.112, 0.050, 0.034), 2)

    # Cooling louvres and a small instrument port.
    p.box((0, -0.52, 0.164), (0.100, 0.30, 0.022), 8)
    p.louvres((-0.046, -0.62, 0.160), (0.046, -0.42, 0.196), 3, mat=1)
    p.cyl((0, -0.86, 0.098), 0.038, 0.040, axis='Z', seg=10, mat=3)
    p.cyl((0, -0.86, 0.114), 0.029, 0.016, axis='Z', seg=10, mat=7)

    p.box((0, 0.92, 0.100), (0.055, 0.18, 0.022), 6)   # stencil band
...
def nose(mats, coll):
    ...
    stations = [(0.02, END_HW, END_HZ), (-0.22, 0.055, 0.066),
                (-0.46, 0.041, 0.050), (-0.66, 0.024, 0.030),
                (-0.78, 0.012, 0.014)]
    p.loft([(y, hull_profile(hw, hz)) for y, hw, hz in stations],
           axis='Y', mat=0)

    # Reinforcing collars where a homemade nose gets banded together.
    for y, r in ((-0.20, 0.062), (-0.48, 0.045)):
        p.tube((0, y, 0), r, 0.012, 0.028, axis='Y', seg=10, mat=1)

    # The spike: a long tapering probe, brass ferrule at its root.
    p.cyl((0, -0.86, 0), 0.015, 0.175, axis='Y', seg=8, mat=3)
    p.cyl((0, -1.10, 0), 0.010, 0.315, axis='Y', seg=8, mat=2,
          radius_top=0.002)
    for sx in (-1, 1):
        p.box((sx * 0.020, -0.80, 0), (0.012, 0.13, 0.038), 2)
...
def boom(mats, coll):
    ...
    stations = [(0.0, END_HW, END_HZ), (0.42, 0.048, 0.058),
                (0.90, 0.038, 0.045), (1.40, 0.031, 0.036),
                (L, 0.028, 0.032)]
    p.loft([(y, hull_profile(hw, hz)) for y, hw, hz in stations],
           axis='Y', mat=0)
    ...
    for y in (0.42, 0.98, 1.56):
        r = 0.052 - 0.014 * (y / L)
        p.tube((0, y, 0), r + 0.010, 0.013, 0.036, axis='Y', seg=10, mat=1)
        for sx in (-1, 1):
            p.box((sx * (r + 0.014), y, 0), (0.021, 0.048, 0.042), 2)

    # A pair of external control cables running the length of the boom.
    for sz in (-1, 1):
        p.cyl((0.0, L * 0.5, sz * 0.048), 0.007, L * 0.92, axis='Y', seg=6,
              mat=2)

    p.box((0, 0.10, 0.074), (0.042, 0.16, 0.018), 6)
...
def tail_hub(mats, coll):
    ...
    p.tube((0, 0, 0), R, 0.026, 0.032, axis='Z', seg=14, mat=1)
    p.tube((0, 0, 0), 0.062, 0.016, 0.026, axis='Z', seg=10, mat=1)
    for i in range(5):
        a = 2 * math.pi * i / 5
        rmid = (0.062 + R - 0.026) / 2
        p.box((rmid * math.cos(a), rmid * math.sin(a), 0),
              (R - 0.062 - 0.026, 0.017, 0.014), 1,
              rot=Matrix.Rotation(a, 4, 'Z'))
    p.cyl((0, 0, 0), 0.044, 0.056, axis='Z', seg=12, mat=3)
    p.cyl((0, 0, 0), 0.017, 0.098, axis='Z', seg=8, mat=2)
