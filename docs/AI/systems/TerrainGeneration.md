...
symptoms:
  ...
  - "the nomad buildings in a new world stand on top of each other with no streets between them"
...
| `NomadSettlementGenerator`, `NomadSettlementPlacer` | [NomadSettlement/](Assets/Game/Scripts/World/ProceduralGeneration/NomadSettlement), Editor/Environment/ | Banded prefab scatter with measured radii; site search + chunk-scene writer for the nine towns |
...
- **A nomad town cannot grow past its chunk.** The whole site disc has to lie inside ONE 500 m chunk (`InsideOneChunk`), because everything the town makes goes into that one chunk scene — so a site of radius R can only be centred in the `500 − 2R` square at the chunk's middle, and that square still has to be level and 800 m from every other town. `MaxSiteRadius` (165 m) is that ceiling, not a taste knob, and a plan that measures larger is scaled down as a whole to fit rather than shipped unplaceable. Today's sites: Small 111 m, Medium 137 m, Large 165 m (clamped from 187 m).
