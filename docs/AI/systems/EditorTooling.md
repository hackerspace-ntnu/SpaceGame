...
| Tools/Build {Dragon Bazooka, Gravel Blaster, Laser Staff, Sucker Puncher, Wrist Blade, Flame Gauntlet} Artifact | [DragonBazookaBuilder.cs](Assets/Game/Editor/AssetPipeline/DragonBazookaBuilder.cs) · [GravelBlasterBuilder.cs](Assets/Game/Editor/AssetPipeline/GravelBlasterBuilder.cs) · [LaserStaffBuilder.cs](Assets/Game/Editor/AssetPipeline/LaserStaffBuilder.cs) · [SuckerPuncherBuilder.cs](Assets/Game/Editor/AssetPipeline/SuckerPuncherBuilder.cs) · [WristBladeBuilder.cs](Assets/Game/Editor/AssetPipeline/WristBladeBuilder.cs) · [FlameGauntletBuilder.cs](Assets/Game/Editor/AssetPipeline/FlameGauntletBuilder.cs) | One artifact each: prefab + item asset, plus its own extras (bazooka rockets and shake assets, `GravelBlastShake.asset`, the lightning VFX prefab, the shock-ring material) |
...
| Tools/SpaceGame/Player/Build Upper Body Layer | [PlayerUpperBodySetup.cs](Assets/Game/Editor/PlayerUpperBodySetup.cs) | Rebuilds the two masked layers inside `AstronautArmature.controller`: `Upper Body` (hold poses, mirrored twins, gauntlet raises) and `Worn Left` (the left arm alone, for a device on each forearm). Then **Build Gestures** ([PlayerGestureBuilder.cs](Assets/Game/Editor/PlayerGestureBuilder.cs)) imports the clips `astronaut/gestures.py` exports and rebuilds the one-shots on `Upper Body`: `Stab`/`Punch` as `{Name} Right`/`{Name} Left` (a blend tree on `AimPitch`, the left one the mirrored twin picked by `HoldMirror`) and one state per emote in `PlayerEmotes.Table` (`Wave`, `Cheer`, `Shrug`, `Flex`), each entered on its own trigger, every other Any State transition guarded with `Gesturing == false`; also puts `PlayerEmotes` on the player prefab. Idempotent |
