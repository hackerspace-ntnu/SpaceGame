...
| Item scanner | [ItemScanner/ItemScannerArtifact.cs](Assets/Game/Scripts/Items/Artifacts/ItemScanner/ItemScannerArtifact.cs) | **Owner**, all-cosmetic. Use toggles power; finds loose salvage in 50 m from a registry, not an OverlapSphere. [ScanBeacon](Assets/Game/Scripts/Items/Artifacts/ItemScanner/ScanBeacon.cs) registers, [ScanTarget](Assets/Game/Scripts/Items/Artifacts/ItemScanner/ScanTarget.cs) types it, [ItemScannerScreen](Assets/Game/Scripts/Items/Artifacts/ItemScanner/ItemScannerScreen.cs) draws it. The **only** gauntlet worn turned off its own model's frame: `GauntletFit.rollDegrees` 180, because the hand-edited `.blend` stands its console on one flank — see [BodyEquipment.md](BodyEquipment.md) |
