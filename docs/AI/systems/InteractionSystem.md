...
- **The interact button has two meanings, and the target picks.** Operate the thing, or — when there is nothing to operate and it implements [`IRetrievable`](Assets/Game/Scripts/Gameplay/Interaction/Core/IRetrievable.cs) — **pick it up**. `Interactor.PressPicksUp` is that question, static and asked in exactly two places: the press, and `IsActionable` (the crosshair). Picking up used to be a key of its own, Q, so the two halves of one verb sat on two buttons — salvage on right mouse, a lantern the player had put down on Q — and Q is also the left gauntlet's trigger, so the pocket gesture fired a worn device (`GDC-L1-UX-0004`, `GDC-L1-UX-0005`). The primary verb wins the press; something that both operates *and* comes back puts the operating half on LMB (`ISecondaryInteractable`).
- **Prompts are default-on.** [`InteractionPromptResolver`](Assets/Game/Scripts/Gameplay/Interaction/Core/InteractionPromptResolver.cs) composes, most specific first: an authored [`InteractionPrompt`](Assets/Game/Scripts/Gameplay/Interaction/Core/InteractionPrompt.cs) component → [`IInteractionReadout`](Assets/Game/Scripts/Gameplay/Interaction/Core/IInteractionReadout.cs) (live label/prompt/0-1 bar/value text) → the humanised type name with noise suffixes stripped (`DoorInteraction` → "Door"). Default prompt `"RMB: interact"`, plus `"   LMB: use"` for an `ISecondaryInteractable`, or `"RMB: pick up"` outright when the same button's only meaning is to take the thing back. **No `InteractionPrompt` is authored on any prefab or scene object** (GUID grep) — every label in the game today comes from a readout or from the type name, so a type whose name does not describe the thing is a type nobody can name at the crosshair.
...
- **The press shows on the presser's body.** After `Interact` (or `Retrieve`), `Interactor` raises a [`CharacterMoment`](Assets/Game/Scripts/Presentation/Animation/CharacterMoment.cs) on the player's `BodyLanguage`: `Interacted` (a reach to operate) unless the target implements [`IInteractionMoment`](Assets/Game/Scripts/Gameplay/Interaction/Core/IInteractionMoment.cs) — `PickupableItem` says `PickedUp`; mounts, seats, boarding, launch, interior entrances, terminals, dialog and petting say `None`, because they move the body, take the camera or have their own motion. It plays whether or not the server then agrees, like the press's sound; the reaction table decides the gesture ([HumanoidAnimation.md](HumanoidAnimation.md)).
...
| `ISecondaryInteractable` | [Core/ISecondaryInteractable.cs](Assets/Game/Scripts/Gameplay/Interaction/Core/ISecondaryInteractable.cs) | Opposite action on Use/LMB (ease vs haul). Also where a placeable's *operating* verb goes, since RMB is its pick-up. |
| `IRetrievable` | [Core/IRetrievable.cs](Assets/Game/Scripts/Gameplay/Interaction/Core/IRetrievable.cs) | "Take it back into the inventory", on the same RMB — reached only when `CanInteract()` is false. Implement alongside `IInteractable`; `ResolveAlongRay` walks that interface only. See [Placeables](Placeables.md). |
...
2. `VisorReticle` resolves that into label/prompt/bar and fades its info box in — at its own fixed spot under the crosshair, while the bracket snaps onto the target. The box says *what*, the bracket says *where*; only the bracket moves.
...
- **On an `IRetrievable`, `CanInteract()` returning true is what takes the pick-up away.** The primary verb wins the button, so giving a placed object a verb of its own leaves the player nothing to press to get the item back — and nothing warns. `PlacedObject.CanInteract` is `false` on purpose; an operating verb goes on `ISecondaryInteractable`.
- **Q is no longer an interaction key.** The `Retrieve` action still exists in the input asset, and `SaddleQuickRelease` is now its only reader — a no-aim gesture for stripping a saddle from up close. It shares the physical key with the left gauntlet's trigger, which is exactly why aimed pick-up left it.
