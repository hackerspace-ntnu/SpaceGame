...
Player-held usable items — every gadget, spell, scanner, throwable and hand tool that occupies a hotbar slot and fires on the shared `Player/Use` action.
...
- **Hold pose** is auto-added: `OnEquipped` adds a `HoldAnimator` when absent unless `UsesHoldPose => false` (worn, not gripped).
...
| `EquipmentController` | [Inventory/Components/EquipmentController.cs](Assets/Game/Scripts/Items/Inventory/Components/EquipmentController.cs) | The only place an artifact is triggered. Hand sockets, grip frame, the four `NetMsg` handlers, hold stream |
...
| Grappling hook | [Gadgets/GrapplingHookArtifact.cs](Assets/Game/Scripts/Items/Artifacts/Gadgets/GrapplingHookArtifact.cs) | **Owner**, `IsContinuous`, `IItemDeferredRestore`. Dart → taut rope → swing/winch ([GrappleRope.cs](Assets/Game/Scripts/Items/Artifacts/Gadgets/GrappleRope.cs) is drawing only) |
...
| Item scanner | [ItemScanner/ItemScannerArtifact.cs](Assets/Game/Scripts/Items/Artifacts/ItemScanner/ItemScannerArtifact.cs) | **Owner**, all-cosmetic. Use toggles power; finds loose salvage in 100 m from a registry, not an OverlapSphere. [ScanBeacon](Assets/Game/Scripts/Items/Artifacts/ItemScanner/ScanBeacon.cs) registers, [ScanTarget](Assets/Game/Scripts/Items/Artifacts/ItemScanner/ScanTarget.cs) types it, [ItemScannerScreen](Assets/Game/Scripts/Items/Artifacts/ItemScanner/ItemScannerScreen.cs) draws it |
...
**Press**
1. `PlayerInputManager.OnUsePressed` → `EquipmentController.OnUse()`.
...
2. Every 1/15 s: `arg { A = slot, B = active }` → `OnRequestHold` → local `PlayHold` → owner `TryHold` if owner-authoritative → `NetToServer(NetMsg.UseItemHold)`.
...
- **Item asset outside `Assets/Game/Resources/Items/`** never registers (`RegistryLoader` does `Resources.LoadAll<InventoryItem>("Items")`), never shows in the dev browser (`I` in dev mode), and every save slot holding it loads empty. `Assets/Game/ScriptableObjects/Items/` holds unreachable duplicates.
...
3. **Prefab** — `Assets/Game/Prefabs/Items/Artifacts/Gadgets/<Name>.prefab`: your script, `PickupableItem`, `DropItemPhysics`, non-kinematic `Rigidbody` + collider, `ItemGrip`, `NetworkObject`. Prefer an editor builder script when it nests an FBX.
