...
| `Drifter_Crumpy` is out of step with `crumpy.fbx` | *Sync Drifter Rigs From FBX (Dry Run)*, 2026-09-25: the FBX has `LeftThumb{Proximal,Intermediate,Distal}` bones the prefab lacks, and the body renderer's 52 bones are not the transforms the FBX's bind poses expect. The count is the same, so nothing errors. The likely cause is a re-export after the left-side weight repair that the prefab never picked up. Not synced and not looked at in a pose: it would change the crumpy's left hand. Run the sync, then pose it and check. | [ArtPipeline](systems/ArtPipeline.md) |
