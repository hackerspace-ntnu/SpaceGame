...
- **One entry point.** Nothing calls `HealthComponent.Damage` directly except the pipeline. Callers use `NetDamage.Apply(target, amount, source)`: it walks `GetComponentInParent<HealthComponent>`, applies locally when `Network.Simulates(health)`, otherwise sends `NetMsg.Damage` to the server. A target with only an `IDamageable` (destructible props) is hit locally with no message. **A hit can be met before it lands.** `HealthComponent.Damage(amount, source, kind)` hands a [`DamageHit`](Assets/Game/Scripts/Gameplay/Health/DamageHit.cs) to each registered [`IDamageFilter`](Assets/Game/Scripts/Gameplay/Health/IDamageFilter.cs) first; a filter may lower `Amount` and set `Defense` (Blocked/Dodged). A defended hit raises `OnDefended`/`AnyDefended` (and `LastDefense`), a hit stopped whole raises no `OnDamage` — so no flinch, no damage number but the "BLOCKED"/"DODGED" label (`NetMsg.Damaged` B). `NetDamage.Apply` takes a `DamageKind` (in `NetMsg.Damage` B); only the Sucker Puncher's direct hit and the Wrist Blade are `Melee` today. The one filter is `MeleeDefense` ([HumanoidAnimation.md](HumanoidAnimation.md) flow 6). `ProvocationModule` counts a blow stopped whole at its `Attempted` amount, so blocking does not make an NPC ignore an attack.
...
| NPC melee / turrets | [CloseCombatModule.cs](Assets/Game/Scripts/agents/Modules/Combat/CloseCombatModule.cs), [RocketLauncherTurret.cs](Assets/Game/Scripts/agents/Modules/Combat/RocketLauncherTurret.cs), [TurretProjectile.cs](Assets/Game/Scripts/agents/Modules/Combat/TurretProjectile.cs) | Same Use/Present split |
...
6. **Death.** `currentHealth <= 0` → `OnDeath`. `HealthReactionModule` plays the death sound/noise, fires `onDeath`, disables `AgentController`, schedules despawn; `EntityLootTable` drops loot; `PlayerController.OnDeath` sets `IsDead` and shows the death screen.
