...
a trigger volume, anything on unnetworked chunk scenery like a settlement's alarm or spawner — it is "true everywhere", so gate on
`Network.Decides` (`!Network.IsNetworked || Network.Server`) instead.
