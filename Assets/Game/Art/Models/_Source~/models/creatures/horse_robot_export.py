"""Export horse_robot.blend to the FBX Unity consumes.
...
time the machine initialises, looking for `Coxa_/Hip_/Knee_/Ankle_/Foot_` chains
and measuring each joint's axle off the `*Pin*` mesh parented to it. Strip the
armature and there is no horse, just a pile of legs.
...
Three things it does that a plain export would not:
  ...
  * **Turns off leaf bones.** Blender otherwise appends a `<bone>_end` child to
    ...
    the first whose name contains "Pin" -- a leaf bone arriving ahead of the
    real pin is a silent wrong answer, not an error.
  * **Exports on the default axis conversion**, so the model's -Y forward
    arrives on Unity's +Z. `LeggedLocomotion` sorts legs by `HomeLocal.x` and
    `.z` in Unity space and the gait patterns derive front/rear and left/right
    from those, so this is what makes the diagonals come out as diagonals.

    blender --background --python horse_robot_export.py
"""
...
SRC = os.path.join(HERE, "horse_robot.blend")
DST = os.path.join(REPO, "Assets", "Models", "Creatures", "Robotic",
                   "Horse", "horse_robot.fbx")


def main():
    if not os.path.exists(SRC):
        raise SystemExit("No model at %s -- run horse_robot.py first." % SRC)
