...
| `ItemGrip` | [Equipped/ItemGrip.cs](Assets/Game/Scripts/Items/Equipped/ItemGrip.cs) | Per-prefab hand, grip point, offsets, `holdSize`/`packSize`, `sizeReference`, `keepColliders` |
...
- **Prefabs owned by builder scripts** (LaserStaff, DragonBazooka, GravelBlaster, RepulsorGauntlet, SuckerPuncher, NetGun, JumpingRod, WingPack…) are replaced wholesale on the next run — tune in the builder, and keep its `holdSize` in sync with the ladder.
...
- Audits: `Tools/SpaceGame/Items/Audit Held Item Poses`, `Tools/SpaceGame/Items/Audit Item Scale Ladder`, `Tools/Generate All Item Icons`. Tests: [HoldPoseTests](Assets/Game/Editor/Tests/HoldPoseTests.cs), [GripFrameTests](Assets/Game/Editor/Tests/GripFrameTests.cs), [HoldLatchTests](Assets/Game/Editor/Tests/HoldLatchTests.cs).
