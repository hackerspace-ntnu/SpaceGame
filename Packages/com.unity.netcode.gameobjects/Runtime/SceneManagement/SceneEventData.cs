...
namespace Unity.Netcode
{
    ...
    internal class SceneEventData : IDisposable
    {
        ...
        internal void WriteSceneSynchronizationData(FastBufferWriter writer)
        {
            ...
            // SPACEGAME PATCH (see Packages/PATCHES.md): remembered, because the loop below may
            // serialize fewer objects than the list holds and the count has to match what follows.
            var networkObjectCountPosition = writer.Position;
            ...
            for (var i = 0; i < m_NetworkObjectsSync.Count; ++i)
            {
                ...
                // SPACEGAME PATCH (see Packages/PATCHES.md): one unserializable object must not cost
                // the joining client the entire world. This loop had no error handling, so the first
                // entry that threw aborted the whole synchronization message and the client was left
                // in a session it could never be told about — with a stack (NetworkObject.Serialize,
                // under WriteSceneSynchronizationData) that names nothing, because the object being
                // written is not in it. An entry left in the spawn table after its GameObject was
                // destroyed, or spawned with a null NetworkManagerOwner, both land here.
                //
                // Skipping the object costs the joiner that one object, which is strictly better
                // than the world, and the report below is what identifies it — it reads only fields
                // that survive the destruction of the object carrying them.
                try
                {
                ...
                {
                    // Back to where this object started: a throw part-way through leaves a partial
                    // record behind, and the client reads the stream by length.
                    writer.Seek(noStart);

                    var report = ReferenceEquals(networkObject, null)
                        ? "a null entry"
                        : $"NID-{networkObject.NetworkObjectId}, hash {networkObject.GlobalObjectIdHash}, " +
                          $"destroyed: {networkObject == null}, no NetworkManagerOwner: {networkObject.NetworkManagerOwner == null}";

                    UnityEngine.Debug.LogError($"[Netcode] Skipped a {nameof(NetworkObject)} while synchronizing " +
                        $"Client-{TargetClientId}: {report}. Despawn an object before destroying it. " +
                        "That object is missing on the joining client.");
            ...
            // SPACEGAME PATCH (see Packages/PATCHES.md): correct the count written above if anything
            // was skipped. Same fixed width as the placeholder, so nothing after it moves.
            if (serializedCount != m_NetworkObjectsSync.Count)
            {
                var afterObjects = writer.Position;
                writer.Seek(networkObjectCountPosition);
                writer.WriteValueSafe(serializedCount);
                writer.Seek(afterObjects);
            }
            if (EnableSerializationLogs)
            {
                UnityEngine.Debug.Log(builder.ToString());
