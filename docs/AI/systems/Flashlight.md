...
- **The pose channel is one per arm and it is not the torch's alone.** `SetWornStyle` is what any worn forearm device asks the body with; the Item Scanner uses it too, to bring its screen up where the wearer can read it. One device per forearm slot, so they cannot fight over an arm — but a lamp on one wrist and a powered scanner on the other are two asks, and the right arm's wins by the same rule two torches do.
