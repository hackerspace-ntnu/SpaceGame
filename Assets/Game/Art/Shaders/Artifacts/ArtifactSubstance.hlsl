...
// Cell noise
// ---------------------------------------------------------------------------------------
// Value noise makes BLOTCHES. Foam is not blotchy — it is packed round cells with dark walls
// between them, and that read is what separates it from wet plaster. A Worley F1 field gives
// exactly that.
//
// The reason it is worth the swap is the SECOND return value. A Worley lookup already knows the
// vector from the sample to the feature point it picked, and that vector IS the height field's
// gradient — so the bump normal comes out of the same cell tests rather than out of two or three
// extra field evaluations. Perturbing a normal along the view direction, which is what this
// replaced in FoamSurface, is not a bump at all: it swings with the camera, and no amount of it
// makes a surface read as domed.
//
// THIS ONE IS 3D, WHERE THE TURBULENCE ABOVE IS TRIPLANAR, and that is a deliberate difference
// rather than an inconsistency. Triplanar blends three independent copies of a field by the
// squared normal, so on any face pointing between two axes the two copies arrive with equal
// weight and AVERAGE. That is survivable for a value field, whose whole output is a smooth
// scalar, and fatal for a cell field, whose entire content is the discontinuity at a cell wall:
// the bubbles mute themselves over half the surface. Sharpening the weights does not help:
// at 45 degrees the two are equal by symmetry at any exponent. A 27-cell 3D lookup costs about
// what the 3x9 triplanar one did, has no blend to mute it and no seam to hide, and its gradient
// is already a world-space 3D vector.

float3 SubstanceHash33(float3 p)
{
...
/// Everything a cell-textured surface needs, in world space, with no UV set.
struct SubstanceCellField
{
    /// 0 on a cell's crown, rising toward 1 in the walls where cells meet.
    float wall;

    /// World-space UNIT direction toward the crown of the cell this point sits on. Subtract it
    /// from a normal to dome the surface: the component along the normal only scales the normal
    /// and normalizes back out, so a cell centred under the fragment correctly tilts nothing.
    ///
    /// IT IS A DIRECTION, NOT A GRADIENT, and a caller that treats it as one gets facets. A
    /// distance field has slope 1 everywhere, so this vector's LENGTH says nothing about how
    /// steep the surface is — used raw it tilts every fragment in a cell by the same angle,
    /// which is a cone. Scale it by a profile of `wall` (`wall` itself gives a paraboloid) to
    /// get a rounded cell.
    float3 uphill;

    /// A 0..1 random for the cell this point belongs to, so a whole bubble can be addressed at
    /// once — which is how a dissolve pops bubbles rather than opening pinholes in all of them.
    float id;
};

/// 3D Worley F1, at `scale` cells per metre.
SubstanceCellField SubstanceCells(float3 position, float scale)
{
    float3 p = position * scale;
    float3 cell = floor(p);
    float3 f = p - cell;

    // WRAPPED BEFORE HASHING, because the world is kilometres across and this is a world-space
    // field. At 14 cells per metre a lump two kilometres out hashes coordinates around 30000,
    // where a float32 frac() has a few digits of mantissa left and the cells degenerate into
    // stripes — the failure only appears far from the origin, which is the worst place to find
    // it. Wrapping repeats the pattern every 4096 cells, about 290 m of foam at this scale, and
    // nobody sees two lumps that far apart as one surface.
    cell = fmod(cell, 4096.0);

    float best = 8.0;
    float3 bestOffset = float3(0.0, 0.0, 1.0);
    float3 bestCell = cell;
    ...
    for (int z = -1; z <= 1; z++)
    {
        for (int y = -1; y <= 1; y++)
        {
            for (int x = -1; x <= 1; x++)
            {
                ...
                float3 d = g + SubstanceHash33(cell + g) - f;
    ...
    // `nearest`, not `distance`: that name is an HLSL intrinsic, and shadowing it would be legal
    // and unreadable.
    float nearest = sqrt(best);

    SubstanceCellField field;
    field.wall = saturate(nearest);
    field.uphill = bestOffset / max(nearest, 1e-4);

    // One hash for the winner rather than one per candidate: the id is only wanted for the cell
    // that actually won, and asking inside the loop would be 27 hashes to throw 26 away.
    field.id = SubstanceHash(bestCell + 0.5);
    return field;
}
...
// Five-band form, for a substance that spans a WHOLE lattice column rather than the dark end of
// one — the storm cloud's body, whose sunlit crown and near-black belly are four lightness rows
// apart. The measured column at hue 270 / chroma 0.046 holds five entries at Oklab L 0.88, 0.80,
// 0.70, 0.58 and 0.43, evenly spaced, and the cloud's darkest band drops off the bottom of it onto
// the grey ramp — which is where SubstanceLadder3's note about "only three usable rows" comes
// from. That note is about the DARK end and still holds; it is not a cap on the ladder.
float3 SubstanceLadder5(float v, float3 a, float3 b, float3 c, float3 d, float3 e)
{
    float index = floor(saturate(v) * 4.0 + 0.5);
    return index < 1.5 ? (index < 0.5 ? a : b)
         : index < 2.5 ? c
                       : (index < 3.5 ? d : e);
}
