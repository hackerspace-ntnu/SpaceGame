...
| `Hidden/PastelQuantize` | [PastelQuantize.shader](Assets/Game/Art/Shaders/Environment/PastelQuantize.shader) | Fullscreen nearest-neighbour palette snap, nothing else. Its `LinearToOklab` must stay in lockstep with `PastelPalette.LinearToOklab`, and its palette arrays are set from C# every frame. `MAX_PALETTE` (256) must equal `MaxPaletteSize` in the render feature: a material's vector-array size freezes the first time it is set, so C# always uploads that many entries and passes `_PaletteCount` as the loop bound. It declares **no shader properties** — palette and blend arrive from the render feature each frame, so the material cannot drift into being a second source of truth |
...
- **Chroma is the axis that decides whether the filter can tell colours apart — not hue count.** With one chroma per lightness the palette is a thin shell in Oklab, so anything muted-but-coloured has no entry near it and falls back to the **grey** ramp: at 100 colours a pale violet-blue sky quantised to flat grey and desert sand snapped to a vivid orange. Splitting chroma into a muted and a vivid variant cut the mean Oklab distance from a sweep of sRGB to the nearest entry by ~34% at the *same* 204 entries (0.0656 -> 0.0432; 95th percentile 0.1460 -> 0.0703), where spending those slots on more hues instead bought almost nothing. **If colours are collapsing together, add a chroma level before adding hues.**
- **The palette count is a per-pixel cost.** The shader walks every entry for every pixel of a fullscreen pass, so 204 entries is roughly twice the work of 100 — and this feature is installed on [Mobile_Renderer.asset](Assets/Game/Settings/Mobile_Renderer.asset) too. The hard ceiling is 256: `MaxPaletteSize` in the render feature and `MAX_PALETTE` in the shader must be raised together, and a material's vector-array size freezes at its first upload.
- **Out-of-gamut palette entries used to be clamped, which moved them.** `LightnessChroma` asks for the same chroma at every hue, but sRGB holds far less chroma in blue than in yellow, so the high-chroma `C 0.17` row fell outside the gamut around cyan/blue. Clamping linear RGB into `[0,1]` there does not just desaturate — it shifts hue and drops lightness, so 13 of the 100 entries rendered darker and dirtier than the ramp says (worst: `#00BCE8` where `#00B8D7` was authored). `PastelPalette.FitChroma` now binary-searches the largest in-gamut chroma instead. **Raising any chroma in the ramp is therefore not linear in effect** — past the gamut edge it does nothing at all for that hue.
- **A smooth screen-space gradient bands into rings under the quantizer.** The pastel filter snaps to 204 flats, so any gentle full-screen ramp becomes visible contours — the "round bright middle" complaint traced to the **Vignette** override on `SampleSceneProfile` (now inactive), not to the helmet visor overlay or glass distortion, which were already off everywhere. Suspect post-processing overrides before the filter itself.
