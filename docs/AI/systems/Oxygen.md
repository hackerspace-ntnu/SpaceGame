...
symptoms:
  ...
  - "there is one spare oxygen tank on the ship's gear wall however many of us landed in it"
...
- **A display copy has no scripts, so it cannot draw its own gauge.** `DisplayCopy.Strip` takes every MonoBehaviour off, including `DockableSupply`. `RefreshTankVisual` therefore binds a `SupplyGauge` on the **copy** — a handle resolved by child name (`Gauge_Anchor`) rather than by component — and `DrawFill` climbs it while the bottle fills. Never the copy's own component, which is gone. The pack mat and the ship's gear wall have exactly the same problem and take exactly the same answer, from `PackContainer`; see [SupplyCharge.md](SupplyCharge.md).
...
- **The lamps are found by MATERIAL NAME, not by a typed submesh index.** These models are one mesh per PART and up to nine materials deep, so the emissive submesh's index is an accident of the export order. A stale index paints the enamel instead of the lamp, which looks like a broken shader rather than like a wrong number. `OxygenGearBuilder` used to be the exception, carrying a hand-typed `TankGaugeIndex = 0`; it resolves `Mat_Emissive_Green_CRT` by name like everything else now, which is also what let the battery be given a gauge at all — nobody had to know where in its nine materials the lit one had landed.
...
**Change where the supplies start** — `OxygenGearBuilder.RouteIntoTheGame` stocks both containers: the ship's gear wall (one battery fixed, one tank **per crew member**) and the expedition rig (one tank). Three authored lists that each container's own builder reads forward, so a rebuild of either keeps them. Anything that should scale with the party goes in `perCrewItems`; anything the ship needs exactly one of goes in `startingMainItems`. The builder's `Verify` re-reads all three off disk, and `WallInventoryTests.TheShippedGearWallCarriesOneBatteryAndATankPerCrewMember` pins them.

**Put a plant somewhere other than the ship** — the prefab has no `NetworkObject`, so whatever places it must add one, and `SaveablePolicy` will then wire its saver itself.
