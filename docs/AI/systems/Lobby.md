...
4. **Local change.** Cycler/team press → view repaints locally *now* → `PublishSuitColor` / `PublishTeam` / `PublishTeamColor` → debounced `UpdatePlayerAsync` → everyone else sees it on their next poll.
...
| Player data (name, suit, team, team colour) | **Each member writes their own** via `UpdatePlayerAsync`. Team colour is therefore an *opinion*, resolved identically on every peer as the highest stamp on that team (ties → earliest lobby order, strict `>`) |
