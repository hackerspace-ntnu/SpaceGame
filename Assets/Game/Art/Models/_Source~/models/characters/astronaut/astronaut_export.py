"""Export astronaut.blend to astronaut.fbx, the PlayerCharacter body.
...
walkers and wrong here. Three things are load-bearing:
  ...
  * **Leaf bones stay OFF**, even though the shipped FBX visibly carries `_end`
    ...
    silent kind of break: `astronaut.fbx` imports as Humanoid with
    ...
    A fresh FBX imports as Generic with NO avatar, so if this file is ever
    recreated rather than overwritten, its importer must be set back to
    Humanoid / Create From This Model. Otherwise the Humanoid clips have nothing
    to retarget through and both Animators silently keep whatever avatar they
    were last given.
...
Two prefabs are built from the body, and BOTH have to be considered when this is
re-exported: `PlayerCharacter.prefab` binds it as a model instance, and
`LobbyPreviewAstronaut.prefab` is an UNPACKED copy generated by
`LobbyPreviewSetup` (Tools > SpaceGame > Menus > Setup Lobby Preview). Because
the lobby copy is unpacked, it does not follow a reimport -- it has to be
regenerated by that tool, or the menu keeps rendering the old mesh.
