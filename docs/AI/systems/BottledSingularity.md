...
layer: items
summary: "A thrown bottle that inhales everything within 8 m, holds it in a white void for 2.5 s, and spits it back out"
paths:
...
symptoms:
  ...
  - "the white room does not exist — the singularity only makes bodies vanish where they stood"
  - "InvalidOperationException: This can only be used during play mode (EditorSceneManager.NewScene)"
...
itself, gulps, collapses — and everything it caught is **taken out of the world** for two and a half seconds,
into a completely white room, before it is thrown back out. No exemptions, not even the thrower.
...
**Two prefabs**, both registered network prefabs: `BottledSingularity.prefab` is the bottle in hand
and in the sand; `SingularityWell.prefab` is thrown. **Six phases, derived from one stamp.**

The server stamps the launch (`launchOrigin`, `launchVelocity`, `seed`, `launchedAt`,
`throwerNetId`) and later the opening (`centre`, `openedAt`). Everything after is
`SingularityWell.PhaseAt`, a pure function of those stamps and the five authored durations, so no
machine holds a phase of its own to get wrong:

| Phase | Default | What it is |
| --- | --- | --- |
| `Flying` | until it lands | On its closed-form arc |
| `Inhaling` | 1.5 s | Pulling. The white sphere grows from nothing to the full 8 m |
| `Flaring` | 0.12 s | Still pulling. The sphere overshoots to 16 m — the gulp |
| `Collapsing` | 0.5 s | Everything caught leaves for the void. The sphere goes black and falls to nothing |
| `Held` | 2.5 s | Nothing to see out here. Everything it took is standing in the white room |
| `Spitting` | 0.08 s | The white ball flashes back and throws the lot out |
...
a deflection by integer avalanche arithmetic; nothing calls `Random` again. The pull is
`RepulsorBlast.DistanceFalloff` and the release is `BlastPush.Apply` — no sweep and no falloff,
because everything is at the centre by then (`SpitVelocity`: a golden-angle fan, then `Scatter`).
...
| `SingularityWell` | The thrown bottle: stamps, flight, sweep, pull, swallow, spit |
...
| `SingularityVoidGuard` | Puts a player back when nothing is holding them in the void |
...
well if this puts them over budget. Every machine traces the arc; only the server stamps `centre`.
...
by their own machine, anything `ITowable` by its owner (the *rider's* for a ridden mount), any other
Rigidbody by the server alone.
...
`InteriorManager.EnterInterior`, which routes a player through their own `PlayerInteriorTransit`.
A body with no `NetworkObject` (chunk scenery) cannot go — the move would not replicate — so
**each machine veils its own copy** instead.

**Spit.** Once, on the first frame of `Spitting`. Out of the void first, flag second (the guard asks
"in the void with no flag"), then the server alone throws through `BlastPush`.

## Multiplayer

Server-authoritative, because it moves other people's bodies and props. Players are the exception:
their movement is owner-authoritative, the well is spawned so "a well is open here" has already
replicated, and each caught player's machine reads it and moves their own body. **No message is sent
for the pull.** The swallow is a discrete latch, so it rides `NetMsg.StatusSet` on the victim's
relay; the move into the void is the server's alone, because scene membership is session state.
...
`"uses"` key. The well is unsaved, and that is a property of what its prefab **carries**:
...
with no well to let them out; `SingularityVoidGuard` puts them back.
...
interior **with no door**, which is what `SingularityVoidGuard` is for.
Everything in it is **URP/Unlit pure white** — a 600 m floor slab and a 400 m sphere with its cull
switched off, and nothing else. Unlit is the mechanism: a *lit* white surface is shaded by the sky
and comes out grey on one side, and the brief is that floor and ceiling are indistinguishable, with
no horizon, wall or corner to orient by. Ambient is flat white so what an occupant *brings* is lit.
...
Unity's unit sphere scaled to `radius * 2` (**diameter is the scale**), coloured white to black
through a `MaterialPropertyBlock`; the ring is a generated annulus.
...
- **The bottle is born inside the thrower.** `IsLandingHit` skips the thrower's root **and** refuses
  ...
  left at `Vector3.zero`, so the singularity opens at the world origin. Skipped **by the flight
  only**. `ExcuseThrower` also pairs `Physics.IgnoreCollision`, or the well collider punts them.
...
- **An editor script creates a scene with `EditorSceneManager.NewScene`** (`Single` only against a
  lone untitled scene, which it otherwise refuses). `SceneManager.CreateScene` throws *"This can
  only be used during play mode"* — how the void builder shipped making `VoidWhite.mat` and nothing
  else. A null `voidInterior` then only warns: `SendToVoid` **veils each body where it stands**, so
  the singularity eats people and takes them nowhere.
- **The veil (chunk scenery only) restores only what it took**, and is re-asserted every tick, or
  LOD and culling write `Renderer.enabled` back mid-hold.
- **`Spit` lifts the veil before it throws** (`BlastPush` is handed a collider), and **`OnDisable`
  gives everything back** — veils, freezes *and* the void — for a well that ends any other way:
  retired over budget, chunk unloaded, world quit.
- **The `Swallowed` flag covers the COLLAPSE too** (`SwallowedFor`): the swallow is the first frame
  of `Collapsing`, so the stay is `collapseSeconds + holdSeconds`. Sized to the hold alone it
  expires with the body still in the room — what `SingularityVoidGuard` reads as stranded.
- **`Swallowed` must never be re-applied to a body already in the void** — `InteriorManager` reads a
  ...
  the body never gets back. `SwallowedStatus.CanApply` refuses a refresh for exactly this reason.
- **Being swallowed is ~6 s somewhere else** — the longest the game takes you out of play. Keeping
  control keeps it off the worst of the commitment axis (`GDC-L1-FEEL-0008`); retune `holdSeconds`
  before adding an escape.
- **`Live`, `Swept` and `Reached` are statics** and survive a world unload and play mode itself.

## Extending

- **A level throw lands inside its own radius** — about 7 m against 8. Lob it.
- **Retuning the shape** is `radius`, `inhaleSeconds`, `pullSpeed`, `pullEdgeShare`; the *drama* is
  `flareSeconds`, `collapseSeconds`, `holdSeconds`, `spitSeconds`. The boundaries are running
  totals, so moving one moves everything after it.
...
- **Anything else that eats a body** applies `StatusKind.Swallowed` rather than growing its own
  flag.
