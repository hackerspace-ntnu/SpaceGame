...
**It is a contest between two ends, and the stronger one wins.** How hard something can haul is its weight times its top speed, so a heavy slow thing and a light fast thing can be evenly matched. Whichever end loses gets **towed** — and how fast it is dragged is the winner's spare strength divided by its own weight, which is just "heavy things move slowly". A wall or a rock face is a special case: it resists everything and hauls nothing, so a rope to one is an anchor and never a winch. Several ropes on the same object add up, and ropes pulling opposite ways cancel, so three people can out-haul one ship and two people can deadlock a crate between them.

**It can drag you, but it cannot launch you.** A rope tied to something stronger will pull you off your feet and along the ground behind it, and that is the point of the item. What stops it becoming a second grappling hook is not a speed limit — it is that there is no winch. Nothing lets you reel yourself in along a rope, so something *else* always has to do the dragging. The grappling hook is a different item and gets to fling you; the leash never does, and there is a test whose entire job is to assert that the code path for it does not exist.

**Nothing snaps just because it is stretched.** Hauling a crate holds a rope permanently overstretched, so any breaking point that survives ordinary towing is one that would never fire in practice. The one way out is to **fight it**: hold your movement squarely away from where the rope is tied and it tears free after a couple of seconds. Leaning sideways earns nothing — it has to be *against* the rope — and the moment you stop, the progress bleeds back. How long it takes scales with what is holding you: another player is quick, the ship takes a while, a crate is nearly instant but never free.

**You cannot tie a rope to open ground.** Aim at bare terrain and you drop what you are holding instead. Rocks, walls and structures still anchor fine — it is only the ground itself, because a rope pinned to open desert is a fence post and this is an item for moving things.
...
**Breaking is decided by whoever is fighting the rope**, and broadcast to everyone. It used to be the server's call, because the verdict was stretch and every machine could measure it. Now the verdict comes from movement input, and only your own machine knows what you are pressing — so you announce it, and everyone else takes your word for it.

A few things people notice and ask about:

- **A creature on a leash strains against it, and can drag you.** A walking animal reports no velocity of its own, so the rope holds it at a standing lean of roughly half a metre past its length rather than snapping taut — that lean *is* the look. Whether it stays there or hauls you along behind it comes down to the contest: rope yourself to something big enough and you are the one being towed.
- **The animal does not know it is leashed.** Its brain keeps pathing exactly as before. It is simply being pulled, and the visible tension between where it wants to go and where it can go is the whole effect. Fighting the rope is something only a *player* can do; an animal has no way to tear free.
- **Vehicles and mounted riders can be towed too.** A rope tied to one asks the machine to come along rather than shoving at it — a rider in a saddle is carried by the vehicle, so pushing on the rider directly achieves nothing at all. Anything that cannot usefully be hauled simply hangs slack, which is the honest answer.
