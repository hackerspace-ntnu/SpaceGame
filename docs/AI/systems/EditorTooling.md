...
| Tools/SpaceGame/Agents/Build Sand Nomad NPCs · Place Sand Nomad Caravan · Retune Sand Nomad Cloth Wind | [NomadPrefabBuilder.cs](Assets/Game/Editor/Agents/NomadPrefabBuilder.cs) | The four `SandNomads` recipes: imports `nomad_{umber,tan,maroon,strawhat}.fbx` as Humanoid, builds `Nomad_*.prefab` with the random-weapon stack, adds the `sand-nomads` caravan to `NpcWorldSim`, syncs network prefabs and wires savers. The second menu reruns only the caravan step. Both `Sync Network Prefabs` and the scene step can raise modal dialogs, so run them from a focused editor |
