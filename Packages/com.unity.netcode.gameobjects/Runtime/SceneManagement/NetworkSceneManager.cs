                    ...
                    if (ServerSceneHandleToClientSceneHandle.TryGetValue(sceneEntry.Key, out var clientSceneHandle))
                    {
                        foreach (var ownerEntry in sceneEntry.Value)
                        {
                            ...
                            if (ScenesLoaded.TryGetValue(clientSceneHandle, out var scene))
                            {
                                ...
                                {
                                    // SPACEGAME PATCH (see Packages/PATCHES.md): skip entries whose object
                                    // is gone. The reference is captured when the ObjectSceneChanged message
                                    // is read, but the object can be destroyed later in that same frame — a
                                    // chunk unload or a despawn — before this runs in PostLateUpdate.
                                    // Dereferencing it then throws, and because the catch below wraps the
                                    // whole loop, every remaining migration in the batch is dropped and then
                                    // cleared: those objects stay in their old scene on this client and are
                                    // destroyed when it unloads, while the server keeps them.
                                    // The IsSpawned prune further down runs after this and only covers
                                    // LocalClientId entries, so it cannot catch these.
