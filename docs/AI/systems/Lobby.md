...
5. **Start.** Host presses Start → `LoadingScreenUI.ShowUntilReady` → `BeginGameAsync`: requires host + a running server, sets `GameState=in-game` (**no lock**), then `NetworkManager.SceneManager.LoadScene`. On failure the loading screen is dismissed and the lobby survives. **The host's call covers the host only** — every other member is moved by Netcode's scene synchronisation and gets its overlay from [NetworkLoadingScreen.cs](Assets/Game/Scripts/Presentation/UI/Pages/NetworkLoadingScreen.cs), which listens for the load event itself. See [UI.md](UI.md).
